...
package com.vsphone.activities.assistant.viewmodels
...
class PhoneAccountCreationViewModel(accountCreator: AccountCreator) :
    AbstractPhoneViewModel(accountCreator) {
            ...
            when (status) {
                AccountCreator.Status.AccountExist, AccountCreator.Status.AccountExistWithAlias -> {
                    ...
                    if (useUsername.value == true) {
                        usernameError.value =
                            AppUtils.getString(R.string.assistant_error_username_already_exists)
                    } else {
                        phoneNumberError.value =
                            AppUtils.getString(R.string.assistant_error_phone_number_already_exists)
                    }
                }
    ...
    private fun isCreateButtonEnabled(): Boolean {
        val usernameRegexp =
            corePreferences.config.getString("assistant", "username_regex", "^[a-z0-9+_.\\-]*\$")
