...
import com.vsphone.utils.PatternClickableSpan
...
class ChatMessageData(val chatMessage: ChatMessage) : GenericContactData(chatMessage.fromAddress) {
    ...
    init {
        ...
        backgroundRes.value =
            if (chatMessage.isOutgoing) R.drawable.chat_bubble_outgoing_full else R.drawable.chat_bubble_incoming_full
    ...
    private fun updateChatMessageState(state: ChatMessage.State) {
        ...
        sendInProgress.value =
            state == ChatMessage.State.InProgress || state == ChatMessage.State.FileTransferInProgress
    ...
    private fun updateContentsList() {
        ...
        for (index in contentsList.indices) {
            ...
            if (content.isFileTransfer || content.isFile || content.isIcalendar) {
                ...
                val spannable =
                    Spannable.Factory.getInstance().newSpannable(content.utf8Text?.trim())
    ...
    private fun updateEphemeralTimer() {
        if (chatMessage.isEphemeral) {
            if (chatMessage.ephemeralExpireTime == 0L) {
                ...
                val remaining =
                    chatMessage.ephemeralExpireTime - (System.currentTimeMillis() / 1000)
