 ...
 */
package com.vsphone.activities.main.settings.viewmodels
...
class LdapSettingsViewModel(private val ldap: Ldap, val index: String) :
    ...
    val ldapCertCheckListener = object : SettingListenerStub() {
        override fun onListValueChanged(position: Int) {
            val params = ldap.params.clone()
            params.serverCertificatesVerificationMode =
                LdapCertVerificationMode.fromInt(ldapCertCheckValues[position])
            ldap.params = params
    ...
    private fun initTlsCertCheckList() {
        ...
        ldapCertCheckIndex.value =
            ldapCertCheckValues.indexOf(ldap.params.serverCertificatesVerificationMode.toInt())
    }
}
