 ...
 */
package com.vsphone.activities.main.conference.viewmodels

import androidx.lifecycle.MediatorLiveData
import androidx.lifecycle.MutableLiveData
import com.vsphone.R
import com.vsphone.VSPhoneApplication.Companion.coreContext
import com.vsphone.activities.main.conference.data.ConferenceSchedulingParticipantData
import com.vsphone.activities.main.conference.data.Duration
import com.vsphone.activities.main.conference.data.TimeZoneData
import com.vsphone.contact.ContactsSelectionViewModel
...
class ConferenceSchedulingViewModel : ContactsSelectionViewModel() {
    ...
    private val listener = object : ConferenceSchedulerListenerStub() {
        override fun onStateChanged(
            ...
            if (state == ConferenceScheduler.State.Ready) {
                ...
                if (scheduleForLater.value == true && sendInviteViaChat.value == true) {
                    ...
                    val chatRoomParams = VSPhoneUtils.getConferenceInvitationsChatRoomParams()
        ...
        override fun onInvitationsSent(
            ...
            if (failedInvitations?.isNotEmpty() == true) {
                ...
                onMessageToNotifyEvent.value =
                    Event(R.string.conference_schedule_info_not_sent_to_participant)
            } else {
    ...
    private fun computeTimeZonesList(): List<TimeZoneData> {
        return TimeZone.getAvailableIDs().map { id -> TimeZoneData(TimeZone.getTimeZone(id)) }
            .toList().sorted()
    }
    ...
    private fun getConferenceStartTimestamp(): Long {
        val calendar = Calendar.getInstance(
            TimeZone.getTimeZone(
                timeZone.value?.id ?: TimeZone.getDefault().id
            )
        )
        calendar.timeInMillis = dateTimestamp
        calendar.set(Calendar.HOUR_OF_DAY, hour)
        calendar.set(Calendar.MINUTE, minutes)
        return calendar.timeInMillis / 1000 // VSPhone expects a time_t (so in seconds)
    }
}
