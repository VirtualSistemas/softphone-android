...
import com.vsphone.activities.main.chat.ChatScrollListener
...
import com.vsphone.activities.main.chat.views.RichEditTextSendListener
...
class DetailChatRoomFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val localSipUri =
            arguments?.getString("LocalSipUri") ?: savedInstanceState?.getString("LocalSipUri")
        val remoteSipUri =
            arguments?.getString("RemoteSipUri") ?: savedInstanceState?.getString("RemoteSipUri")
                    ...
                    if (retryCount == 0) {
                        ...
                        lifecycleScope.launch {
                            withContext(Dispatchers.Default) {
                                val layoutManager =
                                    binding.chatMessagesList.layoutManager as LinearLayoutManager
                                var retryCount = 0
                                do {
        ...
        binding.setAttachFileClickListener {
            if (PermissionHelper.get().hasReadExternalStoragePermission() || PermissionHelper.get()
                .hasCameraPermission()
            ) {
                pickFile()
            } else {
    ...
    private fun showParticipantsDevices() {
        if (corePreferences.limeSecurityPopupEnabled) {
            ...
            val okLabel =
                if (viewModel.oneParticipantOneDevice) getString(R.string.dialog_call) else getString(
                    R.string.dialog_ok
                )
    ...
    private fun showForwardConfirmationDialog(chatMessage: ChatMessage) {
        val viewModel =
            DialogViewModel(getString(R.string.chat_message_forward_confirmation_dialog))
