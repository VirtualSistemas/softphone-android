 ...
 */
package com.vsphone.activities.main.chat.viewmodels
...
import com.vsphone.activities.main.chat.data.ChatMessageAttachmentData
...
class ChatMessageSendingViewModel(private val chatRoom: ChatRoom) : ViewModel() {
    ...
    fun onTextToSendChanged(value: String) {
        sendMessageEnabled.value = value.trim()
            .isNotEmpty() || attachments.value?.isNotEmpty() == true || isPendingVoiceRecord.value == true
        if (value.isNotEmpty()) {
    ...
    fun addAttachment(path: String) {
        ...
        sendMessageEnabled.value = textToSend.value.orEmpty().trim()
            .isNotEmpty() || list.isNotEmpty() || isPendingVoiceRecord.value == true
        if (!corePreferences.allowMultipleFilesAndTextInSameMessage) {
            attachFileEnabled.value = false
        }
    }
    ...
    fun sendMessage() {
        ...
        val message: ChatMessage =
            if (isPendingAnswer.value == true && pendingMessageToReplyTo != null)
    ...
    fun cancelVoiceRecording() {
        ...
        isVoiceRecording.value = false
        sendMessageEnabled.value = textToSend.value.orEmpty().trim()
            .isNotEmpty() == true || attachments.value?.isNotEmpty() == true
    ...
    private fun initVoiceRecordPlayer() {
        ...
        val localPlayer = coreContext.core.createLocalPlayer(
    ...
    private fun updateChatRoomReadOnlyState() {
        isReadOnly.value =
            chatRoom.isReadOnly || (chatRoom.hasCapability(ChatRoomCapabilities.Conference.toInt()) && chatRoom.participants.isEmpty())
    }
}
