...
class ChatMessageContentData(
    ...
    private fun updateContent() {
        ...
        val spannable =
            SpannableString("${AppUtils.getString(R.string.chat_message_download_file)} ($fileSize)")
        ...
        isGenericFile.value =
            !isPdf.value!! && !isAudio.value!! && !isVideo.value!! && !isImage.value!! && !isVoiceRecording.value!! && !isConferenceSchedule.value!!
    ...
    private fun parseConferenceInvite(content: Content) {
        ...
        if (conferenceInfo != null && conferenceUri != null) {
            ...
            conferenceParticipantCount.value = String.format(
                ...
                conferenceInfo.participants.size + 1
            ) // +1 for organizer
    ...
    private fun initVoiceRecordPlayer() {
        ...
        formattedDuration.value = SimpleDateFormat(
            "mm:ss",
            Locale.getDefault()
        ).format(voiceRecordingPlayer.duration) // is already in milliseconds
