 ...
 */
package com.vsphone.activities.main.history.viewmodels
...
import com.vsphone.contact.GenericContactViewModel
...
class CallLogViewModel(val callLog: CallLog, private val isRelated: Boolean = false) :
    GenericContactViewModel(callLog.remoteAddress) {
    val peerSipUri: String by lazy {
        VSPhoneUtils.getDisplayableAddress(callLog.remoteAddress)
    }
    ...
    val secureChatAllowed =
        VSPhoneUtils.isEndToEndEncryptedChatAvailable() && (
    ...
    val conferenceParticipantsData =
        MutableLiveData<ArrayList<ConferenceSchedulingParticipantData>>()
    ...
    fun startChat(isSecured: Boolean) {
        ...
        val chatRoom = VSPhoneUtils.createOneToOneChatRoom(callLog.remoteAddress, isSecured)
