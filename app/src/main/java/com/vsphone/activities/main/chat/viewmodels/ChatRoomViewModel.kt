...
class ChatRoomViewModel(val chatRoom: ChatRoom) : ViewModel(), ContactDataInterface {
    ...
    val groupCallAvailable: Boolean
        get() = VSPhoneUtils.isRemoteConferencingAvailable()
    ...
    fun contactLookup() {
        displayName.value = when {
            basicChatRoom -> VSPhoneUtils.getDisplayName(
                chatRoom.peerAddress
            )
            oneToOneChatRoom -> VSPhoneUtils.getDisplayName(
    ...
    fun areNotificationsMuted(): Boolean {
        val id = VSPhoneUtils.getChatRoomId(chatRoom.localAddress, chatRoom.peerAddress)
    ...
    private fun getParticipantsNames() {
        ...
        for (participant in chatRoom.participants) {
            ...
            participantsList += contact?.name ?: VSPhoneUtils.getDisplayName(participant.address)
            index++
    ...
    private fun updateRemotesComposing() {
        ...
        for (address in chatRoom.composingAddresses) {
            ...
            composing += contact?.name ?: VSPhoneUtils.getDisplayName(address)
        }
