 ...
 */
package com.vsphone.activities.main.recordings.data
...
class RecordingData(val path: String, private val recordingListener: RecordingListener) :
    ...
    init {
        ...
        if (m.matches() && m.groupCount() >= 2) {
            name = m.group(1)
            date = VSPhoneUtils.getRecordingDateFromFileName(m.group(2))
        }
    ...
    private fun initPlayer() {
        ...
        formattedDuration.value = SimpleDateFormat(
            "mm:ss",
            Locale.getDefault()
        ).format(player.duration) // is already in milliseconds
