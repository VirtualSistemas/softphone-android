 ...
 */
package com.vsphone.activities.voip.viewmodels
...
import com.vsphone.activities.voip.data.ConferenceParticipantData
import com.vsphone.activities.voip.data.ConferenceParticipantDeviceData
...
class ConferenceViewModel : ViewModel() {
    ...
    private val conferenceListener = object : ConferenceListenerStub() {
        ...
        override fun onParticipantAdminStatusChanged(
            ...
            val participantData = conferenceParticipants.value.orEmpty()
                .find { data -> data.participant.address.weakEqual(participant.address) }
    ...
    init {
        ...
        activeSpeakerConferenceParticipantDevices.addSource(conferenceParticipantDevices) {
            activeSpeakerConferenceParticipantDevices.value =
                conferenceParticipantDevices.value.orEmpty().drop(1)
        }
    ...
    override fun onCleared() {
        ...
        conferenceParticipantDevices.value.orEmpty()
            .forEach(ConferenceParticipantDeviceData::destroy)
        activeSpeakerConferenceParticipantDevices.value.orEmpty()
            .forEach(ConferenceParticipantDeviceData::destroy)
    ...
    fun toggleRecording() {
        if (conference.value?.isRecording == true) {
            ...
            val path =
                VSPhoneUtils.getRecordingFilePathForConference(conference.value?.currentParams?.subject)
    ...
    fun initConference(conference: Conference) {
        ...
        subject.value = VSPhoneUtils.getConferenceSubject(conference)
