...
import com.vsphone.databinding.VoipConferenceCallFragmentBinding
...
class ConferenceCallFragment : GenericFragment<VoipConferenceCallFragmentBinding>() {
        ...
        conferenceViewModel.conferenceDisplayMode.observe(
            ...
            if (displayMode == ConferenceDisplayMode.ACTIVE_SPEAKER) {
                if (conferenceViewModel.conferenceExists.value == true) {
                    ...
                    val preview =
                        binding.root.findViewById<RoundCornersTextureView>(R.id.local_preview_video_surface)
                    if (preview != null) {
    ...
    private fun adjustActiveSpeakerLayout() {
        if (conferenceViewModel.conferenceCreationPending.value == false) {
            val participantsCount =
                conferenceViewModel.conferenceParticipantDevices.value.orEmpty().size
    ...
    private fun switchToActiveSpeakerLayoutForMoreThanTwoParticipants() {
        ...
        val avatarSize =
            resources.getDimension(R.dimen.voip_conference_active_speaker_miniature_avatar_size)
                .toInt()
