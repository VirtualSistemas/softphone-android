...
class FileUtils {
    companion object {
        ...
        fun clearExistingPlainFiles() {
            ...
            for (
                file in coreContext.context.getExternalFilesDir(Environment.DIRECTORY_PICTURES)
                    ?.listFiles().orEmpty()
            ) {
                if (file.path.endsWith(VFS_PLAIN_FILE_EXTENSION)) {
                    Log.w("[File Utils] Found forgotten plain file: ${file.path}, deleting it")
            ...
            for (
                file in coreContext.context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)
                    ?.listFiles().orEmpty()
            ) {
                if (file.path.endsWith(VFS_PLAIN_FILE_EXTENSION)) {
                    Log.w("[File Utils] Found forgotten plain file: ${file.path}, deleting it")
